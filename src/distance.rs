...
pub fn palette_distance(
    palette_a: &Vec<WeightedColor>,
    palette_b: &Vec<WeightedColor>,
) -> Result<f32> {
    let distance_a_to_b = directional_distance(palette_a, palette_b)?;
    let distance_b_to_a = directional_distance(palette_b, palette_a)?;

    Ok((distance_a_to_b + distance_b_to_a) / 2.0)
}

fn directional_distance(
    from_palette: &Vec<WeightedColor>,
    to_palette: &Vec<WeightedColor>,
) -> Result<f32> {
    ...
    for wc_from in from_palette {
        let min_distance = to_palette
            ...
            .min_by(|a, b| {
                a.partial_cmp(b)
                    .ok_or_else(|| anyhow!("NaN in distance calculation"))?
            })
            .unwrap_or(f32::MAX);
